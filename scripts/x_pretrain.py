...
def main(_):
    ...
    if FLAGS.config.get("wandb_resume_id", None) is None:
        ...
        # resume previous run
        wandb_run = wandb.Api().run(os.path.join(FLAGS.config.wandb.entity, FLAGS.config.wandb.project, FLAGS.config.wandb_resume_id))
        if jax.process_index() == 0:
            wandb.init(
    ...
    # TODO: define multiple train data iterators
    train_data_iter = map(
        shard,
        map(
            process_batch,
    ...
    # create optimizer
    # TODO: add multistep optimizer for multiple batches
    ...
    def loss_fn(params, batch, rng, train=True):
        # TODO: add support for embodiment conditioning
